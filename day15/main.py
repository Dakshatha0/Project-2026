...
def require_user(user, allowed_roles):
...
@app.post('/employees')
def post_employee(employee: Employee, api_key: str = Header()):
    user = authenticate(api_key)
    require_user(user, ["HR"])
...
@app.get('/employees')
def get_employees(api_key: str = Header()):
    user = authenticate(api_key)
    require_user(user, ["HR", "Finance"])
    employees = []
...
def get_employee_data(employee_id: int, api_key: str = Header()):
    user = authenticate(api_key)
    require_user(user, ["HR", "Finance"])
    cursor.execute(
...
def employee_update(employee_id: int, employee: EmployeeUpdate, api_key: str = Header()):
    user = authenticate(api_key)
    require_user(user, ["HR"])
...
def employee_delete(employee_id: int, api_key: str = Header()):
    user = authenticate(api_key)
    require_user(user, ["HR"])
